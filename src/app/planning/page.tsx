...
import { CronMonitoringPanel } from "@/components/planning/cron-monitoring-panel";
...
const planningTracks = [
  {
    ...
    items: [
      ...
      "Calendar feed highlighting venue-space conflicts with direct event detail links.",
      "Weekly digest metrics aligned with executive reporting parity checks.",
    ],
  },
  {
    ...
    items: [
      ...
      "Reviewer queue filters persist via URL parameters for shareable triage views.",
    ],
  },
  {
...
export default async function PlanningPage() {
  ...
  let monitoringSnapshot: Awaited<ReturnType<typeof fetchCronMonitoringSnapshot>> | null =
    null;
  ...
  if (isHQPlanner) {
    try {
      ...
      planningError =
        error instanceof Error ? error.message : "Unable to load planning analytics.";
    }

    if (!planningError) {
      const supabase = createSupabaseServerClient();
      const [goalResult, aiResult, monitoringResult] = await Promise.all([
        supabase
          .from("goals")
        ...
        aiContent = aiRows.map((row) => {
          ...
          return {
            ...
            event: eventValue
              ...
              : null,
          } satisfies AiContentRecord;
        });
      }

      if (monitoringResult.ok) {
        monitoringSnapshot = monitoringResult.snapshot;
      } else {
          ...
          >
            <PlanningAnalyticsClient
              initialData={{
                ...
                slaWarningQueued: planningData.slaWarningQueued,
              }}
            />
          </Suspense>
          {monitoringError ? (
            ...
            <CronMonitoringPanel initialSnapshot={monitoringSnapshot} />
          ) : null}
        </>
      )}
