...
export const detectVenueConflicts = (
  ...
  for (const event of events) {
    const groupKey = `${event.venueId ?? event.venueName ?? "unknown"}::${
      event.venueSpace ?? "general"
    }`;
    const bounds = computeEventBounds(event);
    const existing = eventsByVenue.get(groupKey);
    ...
    if (existing) {
      ...
      eventsByVenue.set(groupKey, {
        venueName: event.venueName ?? "Unknown venue",
        venueSpace: event.venueSpace ?? "General space",
        events: [
          {
            ...event,
            startMs: bounds.startMs,
