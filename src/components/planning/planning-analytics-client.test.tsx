...
describe.sequential("PlanningAnalyticsClient", () => {
  ...
  it("renders seeded dataset guidance when no events are present", () => {
    ...
    expect(markup).toContain("No events found yet");
    expect(markup).toContain("npm run supabase:reset");
  });

  it("shows SLA tiles and ICS download link when analytics data exists", () => {
    useSWR.mockReturnValue({
      data: undefined,
      error: null,
    });
