# Review Pack — v3.1 (final validation before implementation)

Purpose: final adversarial pass on the spec before implementation plan is drafted.

## History
- v1 (original): 29 findings from 4 reviewers, 9 blocking — triggered revision
- v2 (first revision): 6 new findings, 4 blocking — triggered v3
- v3 (second revision): 3 findings, 1 false-positive blocker, 2 tidies — triggered v3.1
- v3.1 (this pack): cosmetic tidies + event_artists SELECT policy loosened

## Focus areas
Verify: (a) v3.1 tidies don't introduce new gaps; (b) event_artists SELECT widening is correct; (c) status-transition trigger behaviour matches canEditEvent + UPDATE RLS; (d) no residual from earlier rounds; (e) spec is implementable as written.

## Workspace conventions
Next.js 15 + Supabase PG + RLS. Action-client respects RLS; admin client bypasses. Vitest tests. British English UI copy. Capability helpers in src/lib/roles.ts.

---

## v3.1 spec (under review)

```markdown
# Office Worker — Propose Rights & Per-Event Edit Scope

**Date:** 2026-04-18
**Status:** Draft v3.1 — revised after two rounds of adversarial review
...
BEGIN
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    IF NOT (
      (OLD.status = 'draft'              AND NEW.status IN ('needs_revisions', 'pending_approval'))
...
## Current production state — key RLS (for comparison)
```sql
-- ─── 5.3: public.events ─────────────────────────────────────────────────────
...
## Current production state — proposal RPC
```sql
-- =============================================================================
