# BaronsHub Website Publishing API (v1)

This document explains how the brand website should pull event data from BaronsHub.

## Values to fill in (copy/paste)
- **Base URL**: `https://baronshub.orangejelly.co.uk`
- **API key**: `<PASTE_BARONSHUB_WEBSITE_API_KEY_HERE>`
  - Used as: `Authorization: Bearer <PASTE_BARONSHUB_WEBSITE_API_KEY_HERE>`
...
- BaronsHub environment variable: `BARONSHUB_WEBSITE_API_KEY`
- Request header:
  - `Authorization: Bearer <BARONSHUB_WEBSITE_API_KEY>`

Keep the API key server-side only. Do not expose it to client-side JavaScript.

## Base URL
Use your BaronsHub deployment URL:
...
Slugs are generated by BaronsHub as:
...
type PublicEvent = {
  ...
  seoSlug: string | null; // optional base slug; in BaronsHub UI we include the date (recommended pattern: <base>-YYYY-MM-DD)
...
## Caching
BaronsHub sends `cache-control` headers (short-lived) on API responses. The website can additionally cache responses server-side if desired, but should respect `updatedAt` for freshness and use `updatedSince` for incremental updates.
...
4) Authorization tab:
   - Type: `Bearer Token`
   - Token: your `BARONSHUB_WEBSITE_API_KEY`
...
```bash
curl -H "Authorization: Bearer $BARONSHUB_WEBSITE_API_KEY" \
  "https://<your-baronshub-domain>/api/v1/events?limit=50"
```

```bash
curl -H "Authorization: Bearer $BARONSHUB_WEBSITE_API_KEY" \
  "https://<your-baronshub-domain>/api/v1/events/by-slug/cask-ale-showcase--aaaaaaa1-0000-4000-8000-000000000001"
```
