...
Example:
`/api/v1/events?endsAfter=2026-01-20T15:00:00.000Z&limit=200`
...
`<slugified-seo-slug-or-public-title>--<eventId>`
...
type PublicEvent = {
  id: string; // uuid
  slug: string; // "<seoSlug-or-title>--<eventId>"
  title: string;
  ...
  seoTitle: string | null; // optional SEO title (<= ~60 chars recommended)
  seoDescription: string | null; // optional SEO description (<= ~155 chars recommended)
  seoSlug: string | null; // optional base slug (lowercase words separated by hyphens)
...
Then try:
- `GET <baseUrl>/api/v1/events?limit=25`
