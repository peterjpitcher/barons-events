...
- Calendar subscribers must already have an authenticated administrator or executive session in the browser where they grab the ICS link. Otherwise the feed returns a 403 due to Supabase RLS.
...
5. Review the Planning Ops executive digest preview (stat pills plus operational coverage cards) to confirm counts align before checking the email.
...
3. Confirm the JSON response includes:
   - `emailsSent > 0`
   - `metrics.statusCounts`, `metrics.conflicts`, and any pending review/approval values.
...
5. Fetch the Planning Ops dashboard (`/planning`) while authenticated as an administrator or executive and visually confirm the status tiles/conflict counts match the cron response.
6. Review the email delivered by Resend (use the staging mailbox) to ensure the digest tiles mirror the same numbers (conflict tiles and pending approval counts should match the dashboard).
...
## Seed Prep (Role Coverage)
1. Open `supabase/seed.sql` and confirm seeded users cover `administrator`, venue-assigned `office_worker`, unassigned `office_worker`, and `executive`.
2. Adjust seeded venue links so assigned office workers only see their venue while unassigned office workers and executives can read globally.
3. Run `npm run supabase:reset` only against disposable local/staging data, then confirm the Planning Ops dashboard, event list, and digest reflect the expected role visibility.
4. Record any venue mappings in your demo notes so they can be reverted after the session.
...
- **ICS feed empty**: Ensure the subscriber is authenticated and that the planning feed returns events (`/api/planning-feed`); confirm Supabase RLS policies allow the user role to view the relevant events.
