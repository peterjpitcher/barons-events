...
drop policy if exists "approvals central planners manage all" on public.approvals;
create policy "approvals central planners manage all"
on public.approvals
as permissive
for all
to authenticated
...
drop policy if exists "ai_content reviewer and central planner read" on public.ai_content;
create policy "ai_content reviewer and central planner read"
...
using (
  exists (
    ...
    where u.id = auth.uid()
      and u.role in ('reviewer','central_planner')
  )
);

drop policy if exists "ai_content central planner manage" on public.ai_content;
create policy "ai_content central planner manage"
on public.ai_content
as permissive
for all
to authenticated
...
drop policy if exists "debriefs central planner manage" on public.debriefs;
create policy "debriefs central planner manage"
on public.debriefs
as permissive
for all
to authenticated
