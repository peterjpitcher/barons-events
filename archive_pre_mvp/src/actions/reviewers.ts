...
export async function assignReviewerAction(
  ...
  if (!profile || !["central_planner", "reviewer"].includes(profile.role ?? "")) {
    return {
...
export async function reviewerDecisionAction(
  ...
  if (!profile || !["reviewer", "central_planner"].includes(profile.role ?? "")) {
    return {
  ...
  const isAssignedReviewer =
    eventRecord.assigned_reviewer_id === profile.id || profile.role === "central_planner";
